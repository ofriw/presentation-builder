...
export function SlidePrimitive({ slide, fragmentStep }: Props) {
  switch (slide.kind) {
    ...
    case "metricTriptych":
      return <MetricTriptych slide={slide} />;
    case "linearScale":
      return <LinearScale slide={slide} />;
    case "fragmentedQuote":
      return <FragmentedQuote slide={slide} fragmentStep={fragmentStep} />;
    case "decisionFunnel":
      return <DecisionFunnel slide={slide} fragmentStep={fragmentStep} />;
    case "pipelineFailure":
      return <PipelineFailure slide={slide} fragmentStep={fragmentStep} />;
    case "iterativeLoop":
      return <IterativeLoop slide={slide} fragmentStep={fragmentStep} />;
    case "parallelResearch":
      return <ParallelResearch slide={slide} fragmentStep={fragmentStep} />;
    case "localVectorBreakthrough":
      return <LocalVectorBreakthrough slide={slide} />;
    case "architectureSplit":
      return <ArchitectureSplit slide={slide} />;
    case "impactDashboard":
      return <ImpactDashboard slide={slide} />;
    case "principleContrast":
      return <PrincipleContrast slide={slide} />;
    case "knowledgeMoat":
      return <KnowledgeMoat slide={slide} />;
    case "closingCta":
      return <ClosingCta slide={slide} />;
  }
}
      ...
      <div className="hero-copy">
        ...
        <div className="title-lockup hero-title">
          {slide.icon && (
            <PhosphorIcon
              icon={slide.icon}
              weight={slide.iconWeight ?? "duotone"}
            />
          )}
          <h1>{slide.title}</h1>
        </div>
  ...
  return (
    <>
      ...
      <div className="scale-story">
        <div
          className="codebase-field"
          aria-label="180 million lines represented as modules"
        >
          {Array.from({ length: 48 }, (_, index) => (
            <span className={index % 11 === 0 ? "hot" : ""} key={index} />
          ))}
        </div>
...
type MetricTileLayout = "natural" | "alignedRows";
...
function MetricTile({
  ...
  metric: Metric;
  layout?: MetricTileLayout;
}) {
  const layoutClass =
    layout === "alignedRows" ? "metric-tile--aligned-rows" : "";

  return (
    <article
      className={`metric-tile ${layoutClass} ${metric.emphasis ?? ""}`.trim()}
    >
      {metric.icon && (
        <div className="metric-tile__icon">
          <PhosphorIcon icon={metric.icon} weight="fill" />
        </div>
      )}
      ...
      {metric.caption && (
        <div className="metric-tile__caption">
...
function LinearScale({ slide }: { slide: Slide }) {
  ...
  const width = (value: number) =>
    `${Math.min(100, (value / content.max) * 100)}%`;
      ...
      <div className="linear-scale-story">
        <div className="scale-axis" aria-hidden="true">
          ...
          {content.references.map((reference, index) => (
            <span
              className={`reference-tick ${index % 2 === 0 ? "high" : "low"}`}
              style={{ left: width(reference.value) }}
              key={reference.label}
            >
              <b>{reference.label}</b>
        ...
        <div
          ...
          aria-label="Linear codebase size comparison in millions of lines of code"
        >
          <div className="scale-row subject">
            <div>
              <strong>{content.subject.label}</strong>
              {content.subject.caption && (
                <small>{content.subject.caption}</small>
              )}
            </div>
            <span
              className="scale-bar"
              style={{ width: width(content.subject.value) }}
            >
        ...
        {content.hyperscaleNote && (
          <p className="hyperscale-note">{content.hyperscaleNote}</p>
        )}
            ...
            <span className={isVisible(index, fragmentStep)} key={item.label}>
              {item.icon && (
                <PhosphorIcon
                  icon={item.icon}
                  weight="bold"
                  className="knowledge-shard-icon"
                />
              )}
      ...
      {(content.footer || content.footerDetail) && (
        <div className="danger-block">
          ...
          {content.footerDetail && (
            <p className="danger-detail">{content.footerDetail}</p>
          )}
        </div>
      )}
    </>
      ...
      <div className="decision-funnel">
        ...
        <div className="filter-stack" aria-label="Decision constraint gates">
          {content.filters.map((filter, index) => {
            ...
            return (
              <div
                className={`filter gate-${index} ${isVisible(index, fragmentStep)}`}
                key={filter}
              >
                {icon && (
                  <PhosphorIcon
...
function PipelineFailure({ slide, fragmentStep }: Props) {
  ...
  return (
    <>
      ...
      <div className="rag-failure">
        <div className="pipeline-diagram" aria-label="Textbook RAG pipeline">
          {content.steps.map((step, index) => (
            <div
              className={`pipeline-step ${stepState(index, content.failureAt, fragmentStep)}`}
              key={step}
            >
              {step}
            </div>
        ...
        {content.scaleMismatch && (
          <p
            className={`scale-mismatch reveal ${fragmentStep > 0 ? "visible" : ""}`}
          >
      ...
      {content.reason && (
        <p
          className={`danger-line reveal ${fragmentStep > 0 ? "visible" : ""}`}
        >
          ✕ {content.reason}
        </p>
      )}
      ...
      <div className="investigation-board">
        ...
        <div className="clue-trail" aria-label="ChunkHound investigation trail">
          {content.trail.map((step, index) => (
            <article
              className={`clue-card ${isVisible(index, fragmentStep)}`}
              key={step.label}
            >
          ...
          <aside
            className={`case-ledger ${isVisible(content.trail.length, fragmentStep)}`}
          >
            <PhosphorIcon icon="ledger" weight="bold" />
            <b>Facts ledger</b>
            {content.ledger.map((item) => (
              <small key={item}>{item}</small>
            ))}
          </aside>
          <strong
            className={`case-answer ${isVisible(content.trail.length + 1, fragmentStep)}`}
          >
            {content.answer}
          </strong>
        </div>
  ...
  return (
    <>
      ...
      <div className="research-hub">
        ...
        {content.spokes.map((spoke, index) => (
          <article
            className={`spoke-card ${isVisible(index, fragmentStep)}`}
            key={spoke.title}
          >
            {spoke.icon && <PhosphorIcon icon={spoke.icon} weight="bold" />}
            <h3>{spoke.title}</h3>
            <p>{spoke.caption}</p>
          </article>
        ))}
      ...
      {content.powerUpSequence && (
        <div className="power-up-strip" aria-label="Power-up sequence example">
          ...
          <div>
            {content.powerUpSequence.split("→").map((step) => (
...
function LocalVectorBreakthrough({ slide }: { slide: Slide }) {
  ...
  return (
    <>
      <Header slide={slide} />
      <div
        className="local-vector-story"
        aria-label="Cloud-scale vector database assumptions constrained inside a developer laptop"
      >
        <section className="cloud-assumption">
          ...
          >
            <path
              ...
              d="M47 168h267c36 0 60-22 60-54 0-33-28-58-69-58-15-30-49-49-90-49-49 0-90 27-106 67-41 1-73 25-73 58 0 14 5 25 11 36z"
            />
            <path
          ...
          <div className="laptop-screen">
            ...
            <div className="local-architecture">
              ...
              <article className="usearch-node">
                ...
                <div>
                  {content.index.shards.map((shard) => (
                    <i key={shard}>{shard}</i>
                  ))}
                </div>
              </article>
  ...
  return (
    <>
      ...
      <div className="impact-dashboard">
        {content.metrics.map((metric) => (
          <MetricTile key={metric.label} metric={metric} layout="alignedRows" />
        ))}
      </div>
      {content.proof?.length > 0 && (
        <div className="proof-rail" aria-label="Community proof">
          <span className="proof-rail__label">Community proof:</span>
          <div className="proof-rail__items">
            {content.proof.map((item) => {
              const label = typeof item === "string" ? item : item.label;
              const icon = typeof item === "string" ? undefined : item.icon;
              const hideLabel =
                typeof item === "string" ? false : item.hideLabel;
              return (
                <span
                  key={label}
...
function List({
  ...
  return (
    <article className={`list-card ${tone ?? ""}`}>
      {icon && <PhosphorIcon icon={icon} weight="bold" />}
      <h2>{title}</h2>
      <ul>
        {items.map((item) => (
          <li key={item}>{item}</li>
...
function isActive(index: number, fragmentStep: number, stepCount: number) {
  return fragmentStep > 0 && index === (fragmentStep - 1) % stepCount
    ? "active"
    : "";
}
